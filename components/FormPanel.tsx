        ...
        <div className="space-y-4">
          {form.ui_schema.elements
            .filter(element => {
              ...
              return fieldName === 'dynamic_checkbox_group' || fieldName === 'dynamic_object';
            })
            .map((element) => {
              const fieldName = element.scope.split('/').pop();
              ...
              return (
                <div key={fieldName}>
                  ...
                  >
                    ...
                    <span>{mapping ? `${mapping.source} - ${mapping.field}` : element.label}</span>
                  </button>
                </div>
              );
            })}
          <div>
            <input
              ...
              placeholder="Select data element to map"
            />
          </div>
        </div>
      </div>
